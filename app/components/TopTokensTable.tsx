      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex items-center justify-between mb-1 flex-wrap gap-3">
          <div className="flex items-center gap-3">
            <div className="text-xs font-medium uppercase tracking-widest" style={{ color: '#22C55E' }}>
          ...
          <div className="flex gap-1 rounded-lg p-1" style={{ backgroundColor: 'rgba(34,197,94,0.06)' }}>
            {TIMEFRAMES.map((tf) => (
              <button
                key={tf.key}
                ...
                style={
                  timeframe === tf.key
                    ? { backgroundColor: '#22C55E', color: '#08100c' }
                    : { color: '#8695a3' }
                }
              >
                {tf.label}
        ...
        <p className="text-xs text-gray-600 mb-5">
          Live DEX pool data on Robinhood Chain via GeckoTerminal. Prices and volume update every few minutes — not real-time to the second.
        ...
        {loading ? (
          ...
          <div className="rounded-lg border px-6 py-10 text-center text-sm text-gray-500" style={{ borderColor: 'rgba(34,197,94,0.2)' }}>
            Live data isn't available right now — check back soon, or browse our{' '}
            <a href="/markets" style={{ color: '#22C55E' }}>curated Markets page</a> for the latest reported figures.
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="text-left text-xs text-gray-600 border-b" style={{ borderColor: 'rgba(34,197,94,0.15)' }}>
