...
function SkeletonHero() {
  return (
    <div style={{ borderRadius: 20, overflow: 'hidden', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(34,197,94,0.1)' }}>
      <div style={{ height: 340, background: 'rgba(255,255,255,0.05)', animation: 'sf-shimmer 1.6s ease-in-out infinite' }} />
      <div style={{ padding: 32 }}>
        <div style={{ height: 10, width: 90, borderRadius: 4, background: 'rgba(34,197,94,0.15)', marginBottom: 16, animation: 'sf-shimmer 1.6s ease-in-out infinite' }} />
...
function SkeletonRow() {
  return (
    <div style={{ display: 'flex', gap: 14, padding: '16px 0', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
      ...
      <div style={{ flex: 1 }}>
        <div style={{ height: 9, width: 60, borderRadius: 4, background: 'rgba(34,197,94,0.1)', marginBottom: 9, animation: 'sf-shimmer 1.6s ease-in-out infinite' }} />
...
export default function NewsFeed({ includeOwn = true }: { includeOwn?: boolean }) {
  const [articles, setArticles] = useState<Article[]>([])
  const [loading, setLoading] = useState(true)
  const [hovered, setHovered] = useState<number | null>(null)

  useEffect(() => {
    // ownArticles.ts doesn't carry an `own` flag (it's shared with FeaturedResearch,
    // which doesn't need one) — tag it here so the "RhoodChain" vs "Cointelegraph"
    // badge below actually reads correctly instead of mislabeling everything.
    const own = includeOwn ? (ownArticles as any[]).map((a) => ({ ...a, own: true })) : []

    const fetchNews = async () => {
      try {
        const res = await fetch('/api/news')
        const data = await res.json()
        const combined = [...own, ...(data.articles || [])]
    ...
    <>
      <style>{`
        ...
        .sf-hero-wrap:hover {
          ...
          box-shadow: 0 20px 60px rgba(0,0,0,.6), 0 0 0 1px rgba(34,197,94,.1);
        }
        .sf-hero-img {
        ...
        .sf-hero-no-img::after {
          ...
          background: repeating-linear-gradient(45deg,transparent,transparent 24px,rgba(34,197,94,.04) 24px,rgba(34,197,94,.04) 25px);
        }
        .sf-hero-gradient {
          position: absolute;
          bottom: 0; left: 0; right: 0;
          height: 65%;
          background: linear-gradient(to top, #0c0f0c 0%, rgba(13,13,18,.75) 50%, transparent 100%);
          pointer-events: none;
        }
        ...
        .sf-hero-wrap:hover .sf-hero-title { color: #22C55E; }
        .sf-read-cta {
        ...
        .sf-card-no-img {
          ...
          background: linear-gradient(135deg, rgba(34,197,94,.06) 0%, transparent 100%);
        }
        ...
        .sf-card-wrap:hover .sf-card-title { color: #22C55E; }
        ...
        .sf-row:hover .sf-row-title { color: #22C55E; }
        ...
        .sf-badge.own { color: #22C55E; background: rgba(34,197,94,.12); }
        .sf-badge.ct  { color: rgba(34,197,94,.6); background: rgba(34,197,94,.06); }
      ...
      <div style={{ padding: '48px 48px 80px', maxWidth: 1400 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 36 }}>
          ...
          <span style={{ fontSize: 11, fontWeight: 700, letterSpacing: '.12em', textTransform: 'uppercase', color: '#22C55E' }}>
            Latest News
          </span>
          <div style={{ flex: 1, height: 1, background: 'linear-gradient(90deg,rgba(34,197,94,.2) 0%,transparent 100%)', marginLeft: 8 }} />
        </div>
        ...
        {loading ? (
                ...
                {[...Array(3)].map((_,i) => (
                  <div key={i} style={{ borderRadius: 14, overflow: 'hidden', border: '1px solid rgba(255,255,255,.06)', background: 'rgba(255,255,255,.02)' }}>
                    ...
                    <div style={{ padding: 16 }}>
                      <div style={{ height: 9, width: 60, borderRadius: 4, background: 'rgba(34,197,94,.1)', marginBottom: 10, animation: 'sf-shimmer 1.6s ease-in-out infinite' }} />
          ...
          <div style={{ borderRadius: 16, padding: '40px 24px', textAlign: 'center', color: 'rgba(255,255,255,.4)', fontSize: 13, border: '1px solid rgba(34,197,94,0.15)', background: 'rgba(34,197,94,0.03)' }}>
            No live news right now — check back soon, or browse our own research on the homepage.
          </div>
        ) : (
                  ...
                  <div className="sf-hero-body">
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 13 }}>
                      <span className={`sf-badge ${hero.own ? 'own' : 'ct'}`}>
                        {hero.own ? 'RhoodChain' : 'Cointelegraph'}
                      </span>
                    ...
                    <span className="sf-read-cta">
                      ...
                      <svg width="13" height="13" viewBox="0 0 13 13" fill="none">
                        <path d="M2.5 10.5L10.5 2.5M10.5 2.5H5.5M10.5 2.5V7.5" stroke="#22C55E" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round"/>
                      </svg>
                    </span>
                  </div>
                </a>
                      ...
                      <div className="sf-card-body">
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 9 }}>
                          <span className={`sf-badge ${article.own ? 'own' : 'ct'}`}>
                            {article.own ? 'RhoodChain' : 'Cointelegraph'}
                          </span>
                  ...
                  {article.image ? (
                    ...
                    <div className="sf-row-no-thumb">
                      <svg width="14" height="14" viewBox="0 0 16 16" fill="none">
                        <path d="M2 12L5.5 8L8 10.5L11 7L14 12H2Z" stroke="rgba(34,197,94,0.35)" strokeWidth="1.2" strokeLinejoin="round"/>
                        <circle cx="5" cy="5" r="1.5" stroke="rgba(34,197,94,0.35)" strokeWidth="1.2"/>
                      </svg>
                    </div>
                  )}
                  <div>
